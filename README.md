...
Apps Script output also includes `collected_at_taipei` so manual inspection can compare the row against Taiwan local time directly.
CSV files generated by the Apps Script runner include a UTF-8 BOM so extracted ZIP contents open correctly in Windows Excel and common text editors.
...
| `collected_at_taipei` | Taiwan local collection time, added by the Apps Script runner for easier manual inspection. | Human-readable local collection time. |
